---
type: hub
domain: current_project
scope: navigation
status: active
last_updated: 2026-05-13
related_hubs:
  - "[[Operations Strategy Hub]]"
  - "[[Technology Stack Hub]]"
  - "[[Business Models Hub]]"
  - "[[Compliance & Regulation Hub]]"
tags:
  - hub
  - navigation
  - current-project
  - source-of-truth
---

# Current Project Navigation Hub

Central deep-link hub for the active Zippy project notes.

Use this note when you want the shortest path between product direction, architecture, frontend surfaces, backend shape, compliance, and execution notes.

## Start Here

- [[Current Architecture Source of Truth]]
- [[Master PRD Distillation for Current Project]]
- [[Backend Structure for Current Project]]
- [[Frontend Architecture for Current Project]]
- [[Codex Context Bundle for Current Project]]

## Product Surfaces

- [[Customer Frontend for Current Project]]
- [[Driver Frontend for Current Project]]
- [[Transport Company Frontend for Current Project]]
- [[Admin and Ops Frontend for Current Project]]
- [[Frontend UI Blueprint for Current Project]]

## Architecture Spine

- [[Current Architecture Source of Truth]]
- [[Backend Structure for Current Project]]
- [[Frontend Architecture for Current Project]]
- [[Current Project Navigation Hub]]
- [[AI Logistics Operating System Roadmap]]

## Data and Operational Truth

- [[Zippy Logistics Operational Core Schema]]
- [[Zippy Logistics Analytics Star Schema]]
- [[Authoritative Database Schema]]
- [[On-Time Delivery Control Tower Strategy for Multimodal Freight]]
- [[Order Lifecycle]]
- [[Notification Taxonomy & Escalation Matrix]]

## Compliance and Trust Layer

- [[Operational Compliance Framework for Indian Logistics Startup 2025-2026]]
- [[Legal Compliance Framework]]
- [[E-way Bill]]
- [[E-way Bill System]]
- [[Proof of Delivery]]
- [[Customer Terms & Privacy Policy Framework]]

## Strategy and Market Layer

- [[South India Tier 2-3 City Pair Freight Atlas Framework]]
- [[South India Trading Pair City Research System]]
- [[South India Logistics Hub Intelligence Framework]]
- [[Industry Hub Cargo Matrix for Triangle Backhaul Optimization]]
- [[Transport Company Network Model]]

## Execution and SOP Layer

- [[SOPs Hub]]
- [[SOP - New Shipment Booking]]
- [[SOP - Assign Vehicle to Order]]
- [[SOP - Verify Shipment Documents]]
- [[SOP - Escalate Delayed Shipment]]
- [[SOP - Handle POD Disputes]]

## Recommended Reading Order for This Project

1. [[Current Architecture Source of Truth]]
2. [[Master PRD Distillation for Current Project]]
3. [[Backend Structure for Current Project]]
4. [[Frontend Architecture for Current Project]]
5. [[Operational Compliance Framework for Indian Logistics Startup 2025-2026]]
6. [[Zippy Logistics Operational Core Schema]]
7. [[On-Time Delivery Control Tower Strategy for Multimodal Freight]]

## Navigation Rule

When you create a new "current project" note:

1. Link it here.
2. Link it to at least one domain hub.
3. Link it to one source-of-truth note.
4. Link it to one execution or data-model note where relevant.
